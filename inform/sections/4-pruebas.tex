...
\paragraph{F1.} ``parses decimal input written with a comma or a dot'' (\texttt{transactions.test.ts}). \texttt{parseAmount("12,50")} y \texttt{parseAmount("12.50")} deben dar ambos \texttt{12.5}. Los usuarios en
...
\paragraph{F2.} ``a validation array maps each field to its message'' (\texttt{api.test.ts}).
...
\paragraph{F3.} ``logout clears storage, not just memory'' (\texttt{auth.test.ts}).
...
\paragraph{F4.} ``groups by calendar day, keeping the server's order'' (\texttt{query.test.ts}).
