...
\textbf{Frontend: \texttt{node:test} + \texttt{node:assert/strict} + \texttt{tsx}.} Se eligió el \emph{runner} de pruebas incluido en Node.js 20 en lugar de Jest o Vitest para no añadir dependencias: \texttt{tsx} carga TypeScript directamente y el comando queda en una línea, {\ttfamily node --import tsx --test "test/**/*.test.ts"}. Las pruebas viven en la carpeta \texttt{frontend/test/}, separadas del código de la aplicación, y cubren la lógica pura de \texttt{app/lib} (dinero, validación de borradores, normalización de errores de la API, sesión, agrupación); simulan \texttt{localStorage} con un \texttt{Map} cuando hace falta.
